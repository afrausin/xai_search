"""
Balanced Optimization - Finding Sweet Spot
Target: Maximize minimum hit rate while maintaining at least 100 short trades
"""
...
# Tiered sector approach - best performing sectors get most shorts
TIER1_SECTORS = {'energy', 'materials', 'utilities'}  # >70% HR in baseline
TIER2_SECTORS = {'healthcare', 'reits'}  # 50% HR in baseline
TIER3_SECTORS = {'industrial'}  # ~36% but high volume


def balanced_backtest(indicators: Dict, ticker: str,
                     ...
                     short_min_zscore: float = 2.5,
                     short_max_zscore: float = 12.0,
                     short_min_beta: float = 0.5,
                     short_max_vol: float = 0.8,
                     target_vol: float = 0.15,
                     inertia_sens: float = 0.8,
                     # Tiered sector approach
                     tier1_only: bool = False,
                     tier1_and_2: bool = False,
                     # Tier-specific adjustments
                     tier1_threshold_adj: float = 0.0,
                     tier2_threshold_adj: float = 0.1,
                     tier3_threshold_adj: float = 0.2,
                     ) -> Dict:
    """
    Balanced backtest with tiered sector approach
    """
    ...
    # Determine if we can short and what adjustments apply
    can_short = False
    sector_adj = 0.0

    if sector in TIER1_SECTORS:
        can_short = True
        sector_adj = tier1_threshold_adj
    elif sector in TIER2_SECTORS and not tier1_only:
        can_short = True
        sector_adj = tier2_threshold_adj
    elif sector in TIER3_SECTORS and not tier1_only and not tier1_and_2:
        can_short = True
        sector_adj = tier3_threshold_adj
    ...
    for i in range(min_idx, n - max_exit):
        ...
        # Short signal with tiered sector approach
        elif can_short and h < short_max_hurst:
            # Apply sector-specific threshold adjustment
            short_thresh = base_threshold_adj * (short_threshold_mult + sector_adj)
...
def test_config(args):
    ...
    for ticker, indicators in precomputed.items():
        result = balanced_backtest(indicators, ticker, **params)
    ...
    # REQUIRE at least 100 short trades for statistical significance
    if result['long_trades'] < 30 or result['short_trades'] < 100:
        return None, None, 0
    ...
    # Score prioritizes minimum hit rate but requires good balance
    # Only consider if trade_ratio >= 0.15 (at least 15% balance)
    if trade_ratio < 0.15:
        return None, None, 0

    score = min_hr * (0.5 + 0.5 * trade_ratio) * (1 - 0.02 * beta_penalty)
...
def main():
    ...
    print("Target: Maximize min HR while maintaining 100+ short trades")
    print("=" * 60)
    ...
    print("\nBuilding balanced parameter grid...")
    configs = []

    # Core parameters near known optimum
    for base_thresh in [2.8, 3.0, 3.2, 3.5]:
        for max_hurst in [0.38, 0.40, 0.42, 0.45]:
            for min_exit, max_exit in [(3, 5), (3, 6), (4, 6), (4, 7)]:
                # Short quality filters - balanced range
                for short_tm in [0.8, 0.9, 1.0, 1.1]:
                    for short_ha in [-0.05, -0.02, 0.0]:
                        for short_em in [0.7, 0.8, 1.0]:
                            for short_md in [2, 3, 4]:
                                for short_mar in [1.0, 1.1, 1.2, 1.3]:
                                    for short_min_z in [2.0, 2.5, 3.0]:
                                        # Tiered approach
                                        for tier_mode in ['all', 'tier1_2', 'tier1']:
                                            params = {
                                                ...
                                                'short_max_zscore': 12.0,
                                                'short_min_beta': 0.3,
                                                'short_max_vol': 0.9,
                                                'target_vol': 0.15,
                                                'inertia_sens': 0.8,
                                                'tier1_only': tier_mode == 'tier1',
                                                'tier1_and_2': tier_mode == 'tier1_2',
                                            }
    ...
    if best:
        ...
        tier_mode = 'tier1' if best_params['tier1_only'] else ('tier1_2' if best_params['tier1_and_2'] else 'all')
        ...
        for k in ['base_threshold', 'max_hurst', 'short_threshold_mult', 'short_max_hurst_adj',
                  ...
                  'short_min_zscore', 'short_min_beta']:
